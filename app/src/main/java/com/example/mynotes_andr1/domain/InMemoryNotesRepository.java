...
public class InMemoryNotesRepository implements NotesRepository {
    ...
    public void loadNoteFolder(NoteFolder folder) {
        ...
        switch (folder.getId()) {
            ...
            case 2:
                noteList.add(new Note("Заметка 6", "Фрагменты создаются Менеджером фрагментов асинхронно. Это значит, что мы не знаем точно, когда именно фрагмент появится и может быть такое (хоть и редко), когда фрагмент пересоздается раньше активити, или уничтожается позже. Если в это время вы обратитесь к Контексту во фрагменте, вы получите NPE", new Date()));
                noteList.add(new Note("Заметка 7", "Одни и те же фрагменты можно переиспользовать в разных частях приложения — это всего лишь экран в рамках какой-либо активити", new Date()));
                break;

            case 3:
                noteList.add(new Note("Заметка 8", "Фрагменты не нужно прописывать в Манифесте, но нужно прописывать для них контейнер в xml", new Date()));
                noteList.add(new Note("Заметка 9", "В коллбэке onCreateView() вы создаете фрагмент из xml", new Date()));
                noteList.add(new Note("Заметка 10", "В коллбэке onViewCreated() вы инициализируете нужные вам элементы", new Date()));
                break;
        }
